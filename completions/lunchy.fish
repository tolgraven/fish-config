...
complete -f -c lunchy -s x -l exact -d 'Force exact (case insensitive) match'
complete -f -c lunchy -n '__fish_lunchy_using_command ls list' -s l -l long -d 'Display absolute paths when listing agents'
complete -f -c lunchy -n '__fish_lunchy_using_command install' -s s -l symlink -d 'Use a symlink for installation'
# Commands
