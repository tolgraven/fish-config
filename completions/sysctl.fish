test -r $fish_complete_path[-1]/sysctl.fish #man page generated
and source $fish_complete_path[-1]/sysctl.fish
...
if type -q -f sysctl
    ...
    if sysctl -h >/dev/null ^/dev/null
        ...
        # OSX sysctl
        function __fish_sysctl_values
            #            sysctl -a ^/dev/null | string replace -a ': ' \t #":" "\t" #tolfix
            for line in (sysctl -a ^/dev/null)
                set split (string split ': ' $line)
                echo -s $split[1] \t (test (count $split) -gt 1; and string sub -l 20 -- $split[2])
            end
        end
