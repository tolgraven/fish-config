function tolfunc --description 'Edit function definition'
    ...
    #function exit_handler --on-job-exit %self --inherit-variable funcname
    #functions -e exit_handler
    #debug "cleaning up on exit from func %s pid %s" $funcname %self
  ...
  (tput smso)(set_color brblue) (set_color normal)   (set_color green)(tput smso) "$funcname" (set_color normal)' #(tput smso)(string sub --start=-6 $clocky)(set_color normal)'
    set -e IFS
    if read --prompt $prompt --right-prompt $right_prompt -c "$init" -s cmd #HERE SHOULD HAVE SOMETHING TO INDICATE IF THESE HAS BEEN ANY CHANGE WHILE EDITING. put in right_prompt somehow hmm?
