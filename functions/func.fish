function func --description 'edit and save function' --argument function force
    ...
    #functions -e __exit #debug "cleaning up %s" $function
    #while set -l index (contains -i $function $__tol_func_editing)
    ...
    and set longest (count (command cat $new)) #debug "rows: %s" $longest #echo "would you like to save?"
    if test -s "$orig" # -s = file non-zero, so existing function
        ...
        if test "$force" = "quiet"
            #echo -n $function
            funcsave $function
        end #move_back_and_kill_lines $longest 0.5 #stop this part if curr func wasnt called from base level
        ...
        #echo -n -s $diff\n; sleep 0.2; move_back_lines (count $diff); commandline -f repaint; echo | grep -Fxv (echo $new) # echo | sd $new #sd diff f stdin-out 'comm also works.but color
        ...
        sleep 0.1
        echo -n -s "."
        sleep 0.3 #commandline -f repaint
        if not test -s $orig
            functions -e $function
        end
    end #functions -e exit_handler
end
