function toled --description 'Inline text editor built on fish funced' --argument file option
    ...
    if not contains -- $opt "-n" "--no-cat"
        and not test -z $init
        #echo "Original file, for comparison while editing: " #nah skip these for now at least
        #cat $filename #| fish_indent --ansi
    end
    toled_bind_mode

    set -l printname (realpath -z --relative-base ~ $filename; or echo $filename)
...
(tput smso)(set_color cyan) (set_color normal)  (set_color brred)(tput smso) '$filename' (set_color normal)'
    ...
    gcp --parents (realpath $filename) ~/.cache/toled/backups/(realpath $filename)/
