...
set grep_args '--color=auto' '-i';  alias grep="grep $grep_args" 
...
switch (hostname)
	case absurd;                      eval (docker-machine env default ^&-) #)^ /dev/null)
		...
		if set -q stderred_on
			#~/.local/lib/libstderred.dylib
			set -q STDERRED_BLACKLIST;    or set -Ux STDERRED_BLACKLIST "fzf|lnav|htop|tmux|zsh|brew|go"
			# set -q DYLD_INSERT_LIBRARIES; or set -Ux DYLD_INSERT_LIBRARIES ~/.local/lib/libstderred.dylib
